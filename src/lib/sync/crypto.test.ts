...
describe('sync crypto', () => {
  it('round-trip: encrypt then decrypt returns identical payload', () => {
    const { encrypted, iv, tag } = encryptPayload(SAMPLE_PAYLOAD);
    const decrypted = decryptPayload(encrypted, iv, tag);
  ...
  it('each encryption produces different ciphertext (random IV)', () => {
    const result1 = encryptPayload(SAMPLE_PAYLOAD);
    const result2 = encryptPayload(SAMPLE_PAYLOAD);
  ...
  it('tampered ciphertext throws on decryption (GCM tag mismatch)', () => {
    ...
    expect(() => decryptPayload(tampered, iv, tag)).toThrow();
  });
  ...
  it('tampered tag throws on decryption', () => {
    ...
    expect(() => decryptPayload(encrypted, iv, tamperedTag)).toThrow();
  });

  it('empty machines object round-trips correctly', () => {
    const empty: SyncPayload = { machines: {} };
    const { encrypted, iv, tag } = encryptPayload(empty);
    const decrypted = decryptPayload(encrypted, iv, tag);

...
* [ ] La segunda publicacion a GitHub Packages no ejecuta `npm publish --provenance` — **Baja**: el job `publish-github-packages` no tiene `--provenance`, a diferencia del job principal de npm
...
| `publish-github-packages` sin `--provenance` | Parcial | Baja | `.github/workflows/release.yml:46` — `npm publish --access public` sin `--provenance`; el registro de GitHub Packages no recibe SLSA attestation | Agregar `--provenance` al publish de GitHub Packages si el registro lo soporta, o documentar como limitacion conocida |
...
| `publish-github-packages` sin `--provenance` | CI/CD | Baja | `.github/workflows/release.yml:46` — la publicacion secundaria a GitHub Packages no incluye SLSA provenance attestation | Agregar `--provenance` o documentar como limitacion; impacto bajo dado que npm es el canal principal |
